...
const isFormValid = computed(() => {
    return isNameValid.value && (!email.value || isEmailValid.value) && isInnValid.value && isOrgValid.value && isPhoneValid.value
})
...
const handleSubmit = async () => {
    ...
    try {
        ...
        if (e.response?.status === 422 && e.response?.data?.errors) {
            ...
            // We can also map others if needed
        }
    } finally {
        loading.value = false
    }
  ...
  <div class="max-w-2xl mx-auto py-8 px-4">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="flex items-center px-6 py-4 border-b border-slate-100 bg-slate-50/50">
            ...
            <h1 class="text-lg font-semibold text-slate-800">Редактирование пользователя</h1>
        </div>
