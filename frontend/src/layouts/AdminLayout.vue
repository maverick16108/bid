...
const navigation = [
  { name: 'Дашборд', href: '/admin/dashboard', icon: HomeIcon },
  { name: 'Заявки', href: '/admin/bids', icon: ClipboardDocumentListIcon },
  { name: 'Пользователи', href: '/admin/users', icon: UserGroupIcon },
]

// Add Moderators link only if Master
const getNavigation = () => {
    const nav = [...navigation]
    if (adminStore.isMaster) {
        nav.push({ name: 'Модераторы', href: '/admin/moderators', icon: ShieldCheckIcon })
    }
    return nav
}
          ...
          <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="-translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="-translate-x-full">
            <DialogPanel class="relative mr-16 flex w-full max-w-xs flex-1">
              ...
              <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-slate-900 px-6 pb-4 ring-1 ring-white/10">
                <div class="flex h-16 shrink-0 items-center justify-center border-b border-white/5 gap-2">
                  <img class="h-8 w-auto" src="/logo/Logo_Novostal_white.webp" alt="Новосталь-М" />
                   <div class="flex flex-col">
                      <span class="text-white font-bold text-sm leading-tight">Новосталь-М</span>
                      <span class="text-[9px] text-slate-400 uppercase tracking-wider font-medium">портал заявок</span>
                   </div>
                </div>
                  ...
                  <ul role="list" class="flex flex-1 flex-col gap-y-7">
                    <li>
                      <ul role="list" class="-mx-2 space-y-2">
                        <li v-for="item in getNavigation()" :key="item.name">
                    ...
                    <li class="mt-auto">
                        <button @click="handleLogout" 
                            class="group -mx-2 flex gap-x-3 rounded-xl p-3 text-sm font-semibold leading-6 text-slate-400 hover:bg-white/5 hover:text-white w-full transition-all duration-200 select-none">
            ...
            <div class="relative">
              <button type="button" class="-m-1.5 flex items-center p-1.5 transition-opacity hover:opacity-80" id="user-menu-button">
                ...
                <span class="hidden lg:flex lg:items-center">
                  <div class="h-8 w-8 rounded-full flex items-center justify-center font-bold border transition-colors duration-200"
                       :class="adminStore.isMaster 
                            ? 'bg-red-100 text-red-700 border-red-200' 
                            : 'bg-blue-100 text-blue-700 border-blue-200'">
                      {{ adminStore.user?.role === 'master' || adminStore.user?.email === 'admin@novostal.ru' ? 'M' : 'A' }}
                  </div>
                  <span class="ml-4 text-sm font-semibold leading-6 text-gray-900" aria-hidden="true">
                    {{ adminStore.user?.name || 'Admin' }}
                  </span>
                </span>
              </button>
            </div>
